...
func main() {
	...
	fmt.Printf(
		"[nfqcooldown] started queue=%d action=%s packet=%s mode=%s cooldown=%s min=%s max=%s jitter=%s burst_interval=%s burst_max_delay=%s whitelist=%d verbose=%v seed=%d\n",
		cfg.QueueNum,
		cfg.Action,
		cfg.Packet,
		cfg.Mode,
...
func statsLoop(ctx context.Context, cfg Config, state *core.State, counters *core.Counters) {
	...
	for {
		...
		case <-t.C:
			...
			fmt.Printf(
				"[nfqcooldown] accepted=%d delayed=%d dropped=%d rejected=%d tracked_ips=%d action=%s packet=%s mode=%s cooldown=%s min=%s max=%s jitter=%s burst_interval=%s burst_max_delay=%s last=%s ip=%s packet_id=%d actual_cooldown=%s elapsed=%s remaining=%s\n",
				accepted,
				delayed,
				dropped,
				rejected,
