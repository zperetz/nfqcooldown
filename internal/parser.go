...
func ParseIPv4PureTCPSYNInfo(data []byte) (PacketInfo, bool) {
	var info PacketInfo
	...
	info.ClientIP = net.IPv4(data[12], data[13], data[14], data[15]).String()
	info.ClientPort = binary.BigEndian.Uint16(tcp[0:2])
	info.ServerPort = binary.BigEndian.Uint16(tcp[2:4])
	info.Seq = binary.BigEndian.Uint32(tcp[4:8])
	return info, true
}

func ParseIPv4PureTCPSYN(data []byte) (string, bool) {
	info, ok := ParseIPv4PureTCPSYNInfo(data)
	return info.ClientIP, ok
}
